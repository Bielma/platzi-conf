...
class MainActivity : AppCompatActivity() {
    ...
    //Solo se usa una vez, para cargar la BD
   fun initFirestore(){
       ...
       for(i in 0 until jsonArr2.length()){
           ...
           cal.timeInMillis = aux.getLong("datetime") *100
           conference.dataTime = cal.time
           conference.speaker = aux.getString("speaker")

           firebase.collection("conferences").document().set(conference)
